...
const AddParticipant = () => {
    ...
    // Get participant record from blockchain
    const getParticipantRecord = async (participantID) => {
        try {
            let resp = await ParticipantService.GetParticipantDetails(participantID);
    ...
    useEffect(() => {
        ...
        // Check the querystring value for participant edit mode
        if (searchParams.get('ref')) {
            console.log(searchParams.get('ref'))
            setIsEdit(true)
            getParticipantRecord(searchParams.get('ref'))
        } else {
            setIsEdit(false)
    ...
    // Generate Unique id for participant record
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (isEdit) {
                resp = await ParticipantService.UpdateParticipant(formData)
            } else {
                formData.dob = selectedDate.toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' });
                formData.participantID = await generateUniqueId()
                resp = await ParticipantService.AddParticipant(formData)
            }
    ...
    const validate = () => {
        if (formData.fName === "" || formData.lName === "") {
            toast.error('Please fill participant details and submit.')
        } else if (!selectedDate) {
            toast.error("Please enter participant Date of Birth.")
        }
    }

    const resetForm = () => {
        // You can't change Name and DOB as it create the unique id for participant
        if (isEdit) {
    ...
    return (
        <>
            <div className="flex flex-col min-h-[calc(100vh_-_150px)] pt-24 sm:pt-20 pb-10 items-center">
                <h1 className="text-3xl font-medium text-zinc-700 my-4 ">{isEdit ? 'Modify Participant data' : 'Add Participant'}</h1>
              ...
              leading-tight outline-none focus:shadow-md"
                                ...
                                onChange={handleChange}
                            >
                                <option value="">Select participant gender</option>
