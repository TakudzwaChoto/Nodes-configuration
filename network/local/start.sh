...
# create the gershom. Specify the channel name and the profile to be used.
./network.sh createChannel -c gershom

# deploy chaincode watercontract on gershom

# Uncomment if chaincode initialization required and add "Initialize" function to the chaincode.
# ./network.sh deployCC -c gershom -ccn watercontract -ccp ../../chaincode -ccl go -cci Initialize
./network.sh deployCC -c gershom -ccn watercontract -ccp ../../chaincode -ccl go -ccs $SEQ_VAL
