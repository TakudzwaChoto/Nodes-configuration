...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating water_quality_data , updating and retrievals,storage Identities"

    cryptogen generate --config=./organizations/cryptogen/crypto-config-water_quality_data , updating and retrievals,storage.yaml --output="organizations"
    res=$?
    ...
    infoln "Creating incentives_contrants Identities"

    cryptogen generate --config=./organizations/cryptogen/crypto-config-incentives_contrants.yaml --output="organizations"
    res=$?
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    while :
    do
      if [ ! -f "organizations/fabric-ca/water_quality_data , updating and retrievals,storage/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
    ...
    createWater_quality_data,UpdatingAndRetrievals,Storage
...
# genesis block is defined using the "three" profile at the bottom
...
## Call the script to deploy a chaincode as a service to the channel
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_water_quality_data , updating and retrievals,storage docker_government docker_incentives_contrants docker_orderer1 docker_orderer2 docker_orderer3
    ...
    # ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/water_quality_data , updating and retrievals,storage/msp organizations/fabric-ca/water_quality_data , updating and retrievals,storage/tls-cert.pem organizations/fabric-ca/water_quality_data , updating and retrievals,storage/ca-cert.pem organizations/fabric-ca/water_quality_data , updating and retrievals,storage/IssuerPublicKey organizations/fabric-ca/water_quality_data , updating and retrievals,storage/IssuerRevocationPublicKey organizations/fabric-ca/water_quality_data , updating and retrievals,storage/fabric-ca-server.db'
    ...
    # ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/incentives_contrants/msp organizations/fabric-ca/incentives_contrants/tls-cert.pem organizations/fabric-ca/incentives_contrants/ca-cert.pem organizations/fabric-ca/incentives_contrants/IssuerPublicKey organizations/fabric-ca/incentives_contrants/IssuerRevocationPublicKey organizations/fabric-ca/incentives_contrants/fabric-ca-server.db'
    # ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/orderer1/msp organizations/fabric-ca/orderer1/tls-cert.pem organizations/fabric-ca/orderer1/ca-cert.pem organizations/fabric-ca/orderer1/IssuerPublicKey organizations/fabric-ca/orderer1/IssuerRevocationPublicKey organizations/fabric-ca/orderer1/fabric-ca-server.db'
    # ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/orderer2/msp organizations/fabric-ca/orderer2/tls-cert.pem organizations/fabric-ca/orderer2/ca-cert.pem organizations/fabric-ca/orderer2/IssuerPublicKey organizations/fabric-ca/orderer2/IssuerRevocationPublicKey organizations/fabric-ca/orderer2/fabric-ca-server.db'
    # ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/orderer3/msp organizations/fabric-ca/orderer3/tls-cert.pem organizations/fabric-ca/orderer3/ca-cert.pem organizations/fabric-ca/orderer3/IssuerPublicKey organizations/fabric-ca/orderer3/IssuerRevocationPublicKey organizations/fabric-ca/orderer3/fabric-ca-server.db'
...
CLI_DELAY=3
# channel name defaults to " gershom "
...
CC_END_POLICY="NA"
# set collection configuration path
